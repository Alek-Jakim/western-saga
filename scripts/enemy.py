...
class Enemy:
    ...
    def get_player_distance_dir(self):
        ...
        dir = (player_pos - enemy_pos).magnitude()
    ...
    def walk_to_player(self):
        distance, dir = self.get_player_distance_dir()
    ...
    def face_player(self):
        ...
        if distance < self.notice_radius:
            if -0.5 < dir.y < 0.5:
                if dir.x < 0:  # player to the left
                    self.status = "left_idle"
                elif dir.x > 0:  # player to the right
                    self.status = "right_idle"
            else:
                if dir.y < 0:  # player top
                    self.status = "up_idle"
                elif dir.y > 0:  # player bottom
